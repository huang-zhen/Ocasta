...
char **backtrace_symbols_ex(void *const *buffer, int size)
{
      ...
      for(x=stack_depth; x>=0; x--){
            memcpy(f_strings, locations[x][0], strlen(locations[x][0]) + 1);
            free(locations[x]);
            final[x] = f_strings;
...
void
backtrace_symbols_fd_ex(void *const *buffer, int size, int fd)
{
        int j;
        char **strings;

        strings = backtrace_symbols_ex(buffer, size);
        if (strings == NULL) {
            perror("backtrace_symbols_ex");
            exit(EXIT_FAILURE);
        }
